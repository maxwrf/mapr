...
class TravelMatrix
  def self.generate(sights, travel_mode)
    ...
    times.each_with_index do |cell, row, col|
      url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=#{cell[:start][0]},#{cell[:start][1]}&destinations=#{cell[:end][0]}%2C#{cell[:end][1]}&mode=#{travel_mode}&key=#{ENV['GOOGLE_API_SERVER_KEY']}"
